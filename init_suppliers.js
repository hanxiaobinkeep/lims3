...
async function initSuppliers() {
  ...
  try {
    console.log('开始创建供应商管理相关表...');
    
    // 创建供应商表
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS lims_db.suppliers (
    ...
    // 创建供应商资质表
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS lims_db.supplier_qualifications (
        id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    // 创建供应商评价表
    await connection.execute(`
      CREATE TABLE IF NOT EXISTS lims_db.supplier_evaluations (
        id INT PRIMARY KEY AUTO_INCREMENT,
        supplier_id INT NOT NULL COMMENT '供应商ID',
    ...
    // 插入示例数据
    await connection.execute(`
      INSERT INTO lims_db.suppliers (supplier_code, supplier_name, short_name, supplier_type, contact_person, contact_phone, is_qualified, status) VALUES
      ('SUP001', '上海化工原料有限公司', '上海化工', '原料', '张先生', '13800138001', 1, 'active'),
      ('SUP002', '北京试剂科技公司', '北京试剂', '试剂', '李女士', '13900139002', 1, 'active'),
      ('SUP003', '广州仪器设备厂', '广州仪器', '设备', '王先生', '13700137003', 1, 'active')
      ON DUPLICATE KEY UPDATE supplier_name = VALUES(supplier_name)
    `);

    console.log('✅ 供应商管理相关表创建完成！');
  } catch (error) {
